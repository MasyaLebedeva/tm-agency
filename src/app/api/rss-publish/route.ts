...
4. БЛОК ПРОДАЖИ T&M AGENCY (включи естественно, БЕЗ заголовка):
   ...
   - НЕ используй заголовок типа "О нас" или "T&M Agency" - просто естественно вплети информацию в текст

5. ПРИЗЫВ К ДЕЙСТВИЮ (CTA, БЕЗ заголовка):
   ...
   - НЕ используй заголовок типа "Призыв к действию" или "Свяжитесь с нами" - просто добавь текст естественно

ВАЖНО - СТРУКТУРА ЗАГОЛОВКОВ:
- Используй markdown формат для заголовков
- H1 (#) - только один раз в начале статьи (основной заголовок)
- H2 (##) - для основных разделов (например: "Пошаговые инструкции", "Практические инструменты", "Экспертные инсайты")
- H3 (###) - для подразделов внутри H2 (например: "Настройка рекламы", "Анализ результатов")
- Каждый раздел должен начинаться с H2 или H3
- Между разделами делай отступы (пустые строки)
- Основной текст пиши обычными абзацами (без #)
...
ТРЕБОВАНИЯ К SEO И ФОРМАТИРОВАНИЮ:
...
- ОБЯЗАТЕЛЬНО структурируй контент заголовками H1, H2, H3 в markdown формате
...
- Блоки продажи T&M Agency и CTA должны быть включены, но БЕЗ явных заголовков типа "О нас" или "Призыв к действию"
- Фокус на полезной информации для читателя, продажа должна быть естественной частью текста
  ...
  try {
    ...
    body: JSON.stringify({
      ...
      messages: [
          { 
            role: 'system', 
            content: 'Ты опытный эксперт по продвижению в Telegram, который пишет максимально полезные статьи для читателей. Ты пишешь человекоподобно, даешь реальную ценность, используешь правильную структуру заголовков (H1, H2, H3) в markdown формате. Включай блоки про T&M Agency и призывы к действию, но БЕЗ явных заголовков для этих блоков - они должны быть естественной частью текста.' 
          },
