...
- НЕ упоминай T&M Agency, не добавляй блоки продажи и призывы к действию
- Фокус только на полезной информации для читателя
  ...
  try {
    ...
    body: JSON.stringify({
      ...
      messages: [
          { 
            role: 'system', 
            content: 'Ты опытный эксперт по продвижению в Telegram, который пишет максимально полезные статьи для читателей. Ты пишешь человекоподобно, даешь реальную ценность, используешь правильную структуру заголовков (H1, H2, H3) в markdown формате. НЕ упоминай агентства, не добавляй блоки продажи и призывы к действию - только полезная информация.' 
          },
